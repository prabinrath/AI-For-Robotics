...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    N = 1
    omg = matrix()
    omg.zero(2*(N+num_landmarks),2*(N+num_landmarks))
    x = matrix()
    x.zero(2*(N+num_landmarks),1)
    omg.value[0][0] = 1
    omg.value[1][1] = 1
    x.value[0][0] = world_size/2
    x.value[1][0] = world_size/2
    ...
    for i in range(len(data)):
        ...
        for j in range(len(measurements)):
            m = measurements[j][0]
            for k in range(2):
                omg.value[2*0+k][2*0+k] += 1/measurement_noise
                omg.value[2*0+k][2*(N+m)+k] += -1/measurement_noise
                omg.value[2*(N+m)+k][2*0+k] += -1/measurement_noise
                omg.value[2*(N+m)+k][2*(N+m)+k] += 1/measurement_noise
                x.value[2*0+k][0] += -measurements[j][k+1]/measurement_noise
                x.value[2*(N+m)+k][0] += measurements[j][k+1]/measurement_noise
        
        omg = omg.expand(2*(N+num_landmarks+1),2*(N+num_landmarks+1),[0,1]+list(range(4,2*(N+num_landmarks+1))),[0,1]+list(range(4,2*(N+num_landmarks+1))))
        x = x.expand(2*(N+num_landmarks+1),1,[0,1]+list(range(4,2*(N+num_landmarks+1))),[0])

        for k in range(2):
            omg.value[2*0+k][2*0+k] += 1/motion_noise
            omg.value[2*0+k][2*(0+1)+k] += -1/motion_noise
            omg.value[2*(0+1)+k][2*0+k] += -1/motion_noise
            omg.value[2*(0+1)+k][2*(0+1)+k] += 1/motion_noise
            x.value[2*0+k][0] += -motion[k]/motion_noise
            x.value[2*(0+1)+k][0] += motion[k]/motion_noise
        
        A = omg.take([0,1], list(range(2,2*(N+num_landmarks+1))))
        B = omg.take([0,1],[0,1])
        C = x.take([0,1],[0])
        omg_temp = omg.take(list(range(2,2*(N+num_landmarks+1))), list(range(2,2*(N+num_landmarks+1))))
        x_temp = x.take(list(range(2,2*(N+num_landmarks+1))), [0])

        omg = omg_temp - A.transpose()*B.inverse()*A
        x = x_temp - A.transpose()*B.inverse()*C

    mu = omg.inverse()*x
    return mu, omg # make sure you return both of these matrices to be marked correct.
