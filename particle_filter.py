...
class robot:
    ...
    def sense(self, noise): #do not change the name of this function
        Z = []
        if noise != 0:
            noise = self.bearing_noise
        # ENTER CODE HERE
        # HINT: You will probably need to use the function atan2()
        for lnd in landmarks:
            dx = lnd[1]-self.x
            dy = lnd[0]-self.y
            Z.append((atan2(dy,dx)-self.orientation + random.gauss(0, noise))%(2*pi))
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        beta = (trans/self.length)*tan(steer)
        
        if abs(beta) < 0.001:
            x = self.x + trans*cos(self.orientation)
            y = self.y + trans*sin(self.orientation)
        else:
            R = trans/beta
        ...
        return r # make sure your move function returns an instance
