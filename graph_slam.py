...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for i in range(len(data)):
        ...
        for k in range(2):
            omg.value[2*i+k][2*i+k] += 1/motion_noise
            omg.value[2*i+k][2*(i+1)+k] += -1/motion_noise
            omg.value[2*(i+1)+k][2*i+k] += -1/motion_noise
            omg.value[2*(i+1)+k][2*(i+1)+k] += 1/motion_noise
            x.value[2*i+k][0] += -motion[k]/motion_noise
            x.value[2*(i+1)+k][0] += motion[k]/motion_noise

        for j in range(len(measurements)):
            m = measurements[j][0]

            for k in range(2):
                omg.value[2*i+k][2*i+k] += 1/measurement_noise
                omg.value[2*i+k][2*(N+m)+k] += -1/measurement_noise
                omg.value[2*(N+m)+k][2*i+k] += -1/measurement_noise
                omg.value[2*(N+m)+k][2*(N+m)+k] += 1/measurement_noise
                x.value[2*i+k][0] += -measurements[j][k+1]/measurement_noise
